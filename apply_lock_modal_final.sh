...
async function refreshStatus(){
  ...
  try{
    ...
    const hasLockst = typeof lockst.isLocked === 'boolean';
    const isUnlocked = hasLockst ? !lockst.isLocked : last.unlocked;
    last.unlocked = isUnlocked || stakingOn;
    setLockIcon(last.unlocked);
    setOK('ic-lock', last.unlocked, 'Wallet unlocked', 'Wallet locked');
...
async function main(){
  ...
  document.getElementById('ic-lock').addEventListener('click', onLockClick);
  ...
  el('pass').addEventListener('keydown', (e)=>{ if(e.key==='Enter') doUnlock(); if(e.key==='Escape'){ el('pass').value=''; el('unlockModal').classList.add('hidden'); } });
...
# ensure modal controls always clickable
python3 - <<'PY'
from pathlib import Path
p=Path('src/renderer/index.html')
s=p.read_text()
s=s.replace('.modal{ position:fixed', '.modal{ position:fixed; -webkit-app-region:no-drag')
s=s.replace('.sheet{ width:360px', '.sheet{ width:360px; -webkit-app-region:no-drag')
s=s.replace('.sheet input{', '.sheet input{ -webkit-app-region:no-drag;')
s=s.replace('.btn{ height:34px', '.btn{ height:34px; -webkit-app-region:no-drag')
p.write_text(s)
print("OK")
PY
