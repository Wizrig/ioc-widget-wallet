    ...
    <div class="modal hidden" id="unlockModal">
      <div class="sheet">
        ...
        <div class="row" style="margin-bottom:10px;">
          <input id="pass" type="password" placeholder="Wallet passphrase" autocomplete="current-password">
        </div>
...
async function refreshStatus(){
  ...
  try{
    ...
    const encStr = String(s.enc ?? '');
    const encLocked = s.lockst?.locked === true || /locked/i.test(encStr) && !/unlocked/i.test(encStr);
    const isUnlocked = stakingOn || (s.wallet?.unlocked_until ?? 0) > 0 || /unlocked/i.test(encStr);
    last.unlocked = isUnlocked;
    setLockIcon(isUnlocked);
    setOK('ic-lock', isUnlocked, 'Wallet unlocked', encLocked ? 'Wallet locked' : 'Wallet');
...
async function main(){
  ...
  el('pass').addEventListener('keydown', (e)=>{ if(e.key==='Enter') doUnlock(); if(e.key==='Escape') el('cancelUnlock').click(); });
  document.getElementById('unlockModal').addEventListener('click', (e)=>{ if(e.target.id==='unlockModal') el('cancelUnlock').click(); });
