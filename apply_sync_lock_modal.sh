...
<head>
  ...
  <style>
    ...
    .app{ background:var(--bg); border-radius:12px; border:none; overflow:hidden; }
    ...
    .chip{ width:28px; height:28px; display:grid; place-items:center; border-radius:8px; background:var(--off); color:#c9d6ea; -webkit-app-region:no-drag; transition:transform .05s; }
    ...
    .modal{ position:fixed; inset:0; background:#0008; display:flex; align-items:center; justify-content:center; z-index:99; }
    .sheet{ width:360px; background:#101826; border:1px solid #223049; border-radius:12px; padding:16px; box-shadow:0 10px 30px #0007; }
    .row{ display:flex; gap:8px; align-items:center; }
    .sheet h3{ margin:0 0 10px; }
    .sheet input{ width:100%; height:34px; padding:0 10px; border-radius:8px; border:1px solid #1a2634; background:#0c1523; color:#e9f1ff; }
    .btn{ height:34px; padding:0 14px; border-radius:8px; border:1px solid #1a2634; background:#0c1523; color:#e9f1ff; cursor:pointer; }
...
<body>
  <div class="app" id="app">
    <div class="topbar">
      ...
      <div class="icons">
        <div class="chip" id="ic-lock" title="Wallet locked"><svg id="svg-lock" viewBox="0 0 24 24"><path d="M12 2a5 5 0 00-5 5v3H6a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2v-8a2 2 0 00-2-2h-1V7a5 5 0 00-5-5zm3 8H9V7a3 3 0 016 0v3z"/></svg></div>
      ...
      <div class="overview">
        ...
        <div class="sendwrap">
          <button class="sendbtn" id="sendBtn">
            <svg viewBox="0 0 24 24"><path d="M2 21l20-9L2 3v7l14 2L2 14z"/></svg>
            Send
          </button>
        </div>
        <div class="syncdock">
...
async function refreshStatus(){
  ...
  try{
    ...
    const encLocked = s.lockst?.locked === true || (s.enc && /locked/i.test(String(s.enc)));
    const isUnlocked = !encLocked;
...
async function lockNow(){
  try{
    await window.ioc.reserve(true, 999999999);
  }catch{}
  try{
    await window.ioc.lock();
  }catch{}
  refreshStatus();
}
function openUnlock(){
...
async function main(){
  ...
  document.getElementById('ic-lock').addEventListener('click', ()=> last.unlocked ? lockNow() : openUnlock());
