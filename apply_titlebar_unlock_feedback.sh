...
.topbar{ -webkit-app-region:no-drag; display:flex; gap:14px; align-items:center; padding:16px 16px 10px 92px; background: linear-gradient(#1e293b55, #00000022); border-bottom:3px solid var(--hdr); }
...
@keyframes shake{10%{transform:translateX(-6px)}20%{transform:translateX(6px)}30%{transform:translateX(-5px)}40%{transform:translateX(5px)}50%{transform:translateX(-4px)}60%{transform:translateX(4px)}70%{transform:translateX(-2px)}80%{transform:translateX(2px)}100%{transform:translateX(0)}}
.err{color:#ff6b6b; font-size:12px; min-height:16px; margin-top:6px}
...
.sheet.shake{ animation:shake .35s linear; }
...
const showUnlock=()=>{ el('unlockErr').textContent=''; el('unlockSheet').classList.remove('shake'); show('unlockModal'); };
...
async function refreshStatus(){
  ...
  try{
    ...
    const weight = s.staking?.weight ?? s.mining?.weight ?? s.staking?.stake ?? s.mining?.stake ?? 0;
...
async function doUnlock(){
  ...
  try{
    ...
    const ok = !!(st.lockst && st.lockst.isLocked === false);
    if(!ok) throw new Error('bad');
    el('pass').value='';
    last.unlocked = true;
    setLockIcon(true); setOK('ic-lock', true, 'Wallet unlocked', 'Wallet locked');
    hideModal(); refreshStatus();
    if(typeof afterUnlock === 'function'){ const fn=afterUnlock; afterUnlock=null; setTimeout(fn, 150); }
  }catch(e){
    el('unlockErr').textContent='Wrong passphrase';
...
async function doSend(){
  ...
  try{
    ...
    hideSend(); setTimeout(refreshStatus, 500);
