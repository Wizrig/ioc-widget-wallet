...
.app{ background:var(--bg); border-radius:12px; overflow:hidden; border:1px solid #1a2330; }
...
.page{ padding:0; }
.overview{ position:relative; background:var(--panel); border:1px solid var(--edge); border-radius:10px; margin:14px; padding:24px 24px 70px; min-height: calc(100vh - 128px); }
...
const hideModal = () => { const m=el('unlockModal'); if(m){ m.classList.add('hidden'); m.style.display='none'; } };
const showModal = () => { if(last.unlocked || last.stakingOn) return; const m=el('unlockModal'); if(m){ m.classList.remove('hidden'); m.style.display='flex'; setTimeout(()=>el('pass')?.focus(),0); } };
...
function setSync(pct, txt){
  ...
  const syncChip = el('ic-sync'); if(syncChip) syncChip.classList.toggle('ok', (pct||0)>=100);
}
