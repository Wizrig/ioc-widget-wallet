...
function dataDir() {
  return path.join(os.homedir(), 'Library', 'Application Support', 'IOCoin');
}
function confPath() {
...
function readCookie() {
  const p = path.join(dataDir(), '.cookie'); // format: user:token
...
function candidateTargets() {
  ...
  const ports = conf.rpcport ? [Number(conf.rpcport)] : [7332, 33765]; // try common ports
  const authPref = [];
  ...
  for (const port of ports) {
    for (const auth of authPref.length ? authPref : [{ type: 'none' }]) {
      out.push({ host, port, auth });
    }
  }
  return out;
}
...
// Register IPC once (no creds exposed to renderer)
